...
#get latest reference application version number
...
#compare with current version number and download newer version files if available
...
if [ "$LATEST_REFERENCE_APPLICATION_VERSION" = "$CURRENT_REFERENCE_APPLICATION_VERSION" ]; then
  ...
  while IFS=$'\n' read -r latest_module; do
    ...
    if [ "$latest_module" = "$current_module" ]; then
      ...
      #delete outdated module version from repo
